    ...
    <li>
      <NextLink href={href ?? ""} legacyBehavior passHref>
        <NavigationMenuLink
          className={cn(
            className,
            "flex flex-col gap-1 items-start justify-center rounded-md text-sm transition-colors focus:outline-none focus:bg-accent focus:text-accent-foreground disabled:opacity-50 disabled:pointer-events-none bg-background hover:bg-accent hover:text-accent-foreground data-[state=open]:bg-accent/50 data-[active]:bg-accent/50 py-2 px-4 group w-full",
          )}
          {...props}
          ref={ref}
        >
          ...
          {Boolean(children) && (
            <p className="leading-snug line-clamp-2 text-muted-foreground">
              {children}
            </p>
          )}
        </NavigationMenuLink>
